...
// Max Depth
// Test Against Leaves
vec4 raycast_branch_depth_3(vec3 ro,vec3 rd,vec3 c,float index) {
	vec4 result = vec4(0.0,0.0,0.0,0.0);
	float hSize = f_treeSize * 0.0625;
	float qSize = f_treeSize * 0.03125;
...
vec4 raycast_branch_depth_2(vec3 ro,vec3 rd,vec3 c,float index) {
	vec4 result = vec4(0.0,0.0,0.0,0.0);
	...
	if (ftl && ftl_i > -1.0) ftl_c = raycast_branch_depth_3(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z - qSize),ftl_i);
	if (ftr && ftr_i > -1.0) ftr_c = raycast_branch_depth_3(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z - qSize),ftr_i);
	if (fbl && fbl_i > -1.0) fbl_c = raycast_branch_depth_3(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z - qSize),fbl_i);
	if (fbr && fbr_i > -1.0) fbr_c = raycast_branch_depth_3(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z - qSize),fbr_i);
	if (btl && btl_i > -1.0) btl_c = raycast_branch_depth_3(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z + qSize),btl_i);
	if (btr && btr_i > -1.0) btr_c = raycast_branch_depth_3(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z + qSize),btr_i);
	if (bbl && bbl_i > -1.0) bbl_c = raycast_branch_depth_3(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z + qSize),bbl_i);
	if (bbr && bbr_i > -1.0) bbr_c = raycast_branch_depth_3(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z + qSize),bbr_i);
...
vec4 raycast_branch_depth_1(vec3 ro,vec3 rd,vec3 c,float index) {
	vec4 result = vec4(0.0,0.0,0.0,0.0);
	float hSize = f_treeSize * 0.25;
	float qSize = f_treeSize * 0.125;
	float finalDepth = 1.0;
	...
	if (ftl && ftl_i > -1.0) ftl_c = raycast_branch_depth_2(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z - qSize),ftl_i);
	if (ftr && ftr_i > -1.0) ftr_c = raycast_branch_depth_2(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z - qSize),ftr_i);
	if (fbl && fbl_i > -1.0) fbl_c = raycast_branch_depth_2(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z - qSize),fbl_i);
	if (fbr && fbr_i > -1.0) fbr_c = raycast_branch_depth_2(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z - qSize),fbr_i);
	if (btl && btl_i > -1.0) btl_c = raycast_branch_depth_2(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z + qSize),btl_i);
	if (btr && btr_i > -1.0) btr_c = raycast_branch_depth_2(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z + qSize),btr_i);
	if (bbl && bbl_i > -1.0) bbl_c = raycast_branch_depth_2(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z + qSize),bbl_i);
	if (bbr && bbr_i > -1.0) bbr_c = raycast_branch_depth_2(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z + qSize),bbr_i);
...
vec4 raycast_root(vec3 ro,vec3 rd) {
	...
	if (ftl && ftl_i > -1.0) ftl_c = raycast_branch_depth_1(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z - qSize),ftl_i);
	if (ftr && ftr_i > -1.0) ftr_c = raycast_branch_depth_1(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z - qSize),ftr_i);
	if (fbl && fbl_i > -1.0) fbl_c = raycast_branch_depth_1(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z - qSize),fbl_i);
	if (fbr && fbr_i > -1.0) fbr_c = raycast_branch_depth_1(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z - qSize),fbr_i);
	if (btl && btl_i > -1.0) btl_c = raycast_branch_depth_1(ro,rd,vec3(c.x - qSize,c.y + qSize,c.z + qSize),btl_i);
	if (btr && btr_i > -1.0) btr_c = raycast_branch_depth_1(ro,rd,vec3(c.x + qSize,c.y + qSize,c.z + qSize),btr_i);
	if (bbl && bbl_i > -1.0) bbl_c = raycast_branch_depth_1(ro,rd,vec3(c.x - qSize,c.y - qSize,c.z + qSize),bbl_i);
	if (bbr && bbr_i > -1.0) bbr_c = raycast_branch_depth_1(ro,rd,vec3(c.x + qSize,c.y - qSize,c.z + qSize),bbr_i);
