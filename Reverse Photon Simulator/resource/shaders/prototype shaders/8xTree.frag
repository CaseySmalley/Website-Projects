...
vec4 raycast_branch_depth_2(vec3 ro,vec3 rd,vec3 c,float index) {
	...
	float hSize = f_treeSize * 0.125;
	float qSize = f_treeSize * 0.0625;
...
vec4 raycast_branch_depth_1(vec3 ro,vec3 rd,vec3 c,float index) {
	...
	bool ftl = rayToAABB(ro,rd,vec3(c.x - hSize,c.y,c.z - hSize),vec3(c.x,c.y + hSize,c.z));
	bool ftr = rayToAABB(ro,rd,vec3(c.x,c.y,c.z - hSize),vec3(c.x + hSize,c.y + hSize,c.z));
	bool fbl = rayToAABB(ro,rd,vec3(c.x - hSize,c.y - hSize,c.z - hSize),vec3(c.x,c.y,c.z));
	bool fbr = rayToAABB(ro,rd,vec3(c.x,c.y - hSize,c.z - hSize),vec3(c.x + hSize,c.y,c.z));
	bool btl = rayToAABB(ro,rd,vec3(c.x - hSize,c.y,c.z),vec3(c.x,c.y + hSize,c.z + hSize));
	bool btr = rayToAABB(ro,rd,vec3(c.x,c.y,c.z),vec3(c.x + hSize,c.y + hSize,c.z + hSize));
	bool bbl = rayToAABB(ro,rd,vec3(c.x - hSize,c.y - hSize,c.z),vec3(c.x,c.y,c.z + hSize));
	bool bbr = rayToAABB(ro,rd,vec3(c.x,c.y - hSize,c.z),vec3(c.x + hSize,c.y,c.z + hSize));
