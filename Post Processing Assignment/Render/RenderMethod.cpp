...
ID3D10EffectShaderResourceVariable* NoiseMapVar = NULL;
ID3D10EffectShaderResourceVariable* BurnMapVar = NULL;
ID3D10EffectShaderResourceVariable* DistortMapVar = NULL;
ID3D10EffectScalarVariable* BurnLevelVar = NULL;
ID3D10EffectScalarVariable* NoiseOffsetVar = NULL;
...
//	|Technique name|  |Method init fn|         |Num Tex|  |Tangents|  |Post-Process|  |for internal use|   |Method Name|
	...
	"PPCutGlassPoly", RM_ParallaxMapping,      2,         true,       true,          0,                // ParallaxMap   
	"PPGrayNoise", RM_TransformColour, 0, false, true ,0,  
	"PPBurn", RM_TransformColour, 0, false, true ,0,
	"PPSpiralDistortion", RM_TransformColour, 0, false, true ,0
};
...
)
{
	...
	else
	{
		...
		else if (materialName.find("GrayNoise") == 0)
		{
			return PPGrayNoise;
		}
		else if (materialName.find("Burn") == 0)
		{
			return PPBurn;
		}
		else if (materialName.find("SpiralDistortion") == 0)
		{
			return PPSpiralDistortion;
		}
		else
		{
			return PixelLitTex;
...
bool InitialiseMethods()
{
	...
	NoiseOffsetVar		= Effect->GetVariableByName( "NoiseOffset" )->AsScalar();
	NoiseMapVar			= Effect->GetVariableByName( "NoiseMap" )->AsShaderResource();
	BurnMapVar			= Effect->GetVariableByName("BurnMap")->AsShaderResource();
	DistortMapVar		= Effect->GetVariableByName("DistortMap")->AsShaderResource();
	BurnLevelVar		= Effect->GetVariableByName("BurnLevel")->AsScalar();

	NoiseMapVar->SetResource(NoiseMap);
	BurnMapVar->SetResource(BurnMap);
	DistortMapVar->SetResource(DistortMap);
...
{
	burnLevel = Mod(burnLevel + 0.01f, 6.28318530718f);
	...
	NoiseOffsetVar->SetFloat(static_cast<float>(rand() % 100) / 100.0f);
	BurnLevelVar->SetFloat(sin(burnLevel) * 0.5f + 0.5f);
	//if (burnLevel > 1.0f) burnLevel = 0.0f;
}
