...
float4 PPTintShader( VS_BASIC_OUTPUT vOut ) : SV_Target
{
	...
	float2 UVScene = float2( (vOut.ProjPos.x - 0.5f) / ViewportWidth, (vOut.ProjPos.y - 0.5f) / ViewportHeight);
	...
	float4 colour = SceneTexture.Sample(PointClamp, UVScene) * DiffuseColour;
	colour.w = 1.0f;
	return colour;
}
...
float4 PPCutGlassShader( VS_NORMALMAP_OUTPUT vOut ) : SV_Target
{
	...
	float2 sceneUV = float2((vOut.ProjPos.x - 0.5f) / ViewportWidth, (vOut.ProjPos.y - 0.5f) / ViewportHeight); // Not 0, fill in the correct values
	//...
...
float4 PPGrayNoiseShader(VS_TEX_OUTPUT vOut) : SV_Target {
		const float NoiseStrength = 0.2f;
		float2 UVScene = float2((vOut.ProjPos.x - 0.5f) / ViewportWidth, (vOut.ProjPos.y - 0.5f) / ViewportHeight);
		float2 NoiseUV = vOut.UV + NoiseOffset;
		float4 texColour = SceneTexture.Sample(PointClamp, UVScene);
		float gray = ((texColour.r + texColour.g + texColour.b) / 3.0f) + 0.5f * (NoiseMap.Sample(TrilinearWrap, NoiseUV).r - 0.5f);
...
float4 PPBurnShader(VS_TEX_OUTPUT vOut) : SV_Target {
	...
	const float GlowAmount = 0.15f;
	const float Crinkle = 0.1f;
	float2 UVScene = float2((vOut.ProjPos.x - 0.5f) / ViewportWidth, (vOut.ProjPos.y - 0.5f) / ViewportHeight);
	float4 burnTexture = BurnMap.Sample(TrilinearWrap, vOut.UV);
	...
	{
		float3 ppColour = SceneTexture.Sample(PointClamp,UVScene);
		return float4(ppColour, 1.0f);
	}
	...
	else
	{
		...
		float4 texColour = SceneTexture.Sample(PointClamp,UVScene - GlowLevel * Crinkle * CrinkleVector);
...
float4 PPSpiralDistortionShader(VS_TEX_OUTPUT vOut) : SV_Target {
	const float LightStrength = 0.025f;
	const float DistortLevel = 0.05f;
	float2 UVScene = float2((vOut.ProjPos.x - 0.5f) / ViewportWidth, (vOut.ProjPos.y - 0.5f) / ViewportHeight);
	float4 distortTexture = DistortMap.Sample(TrilinearWrap, vOut.UV);
	...
	float3 ppColour = light + SceneTexture.Sample(PointClamp, UVScene + DistortLevel * DistortVector);
...
technique10 PPGrayNoise
{
	pass P0
	{
		SetVertexShader(CompileShader(vs_4_0, VSTransformTex()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_4_0, PPGrayNoiseShader()));
...
technique10 PPSpiralDistortion
{
	pass P0
	{
		SetVertexShader(CompileShader(vs_4_0, VSTransformTex()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_4_0, PPSpiralDistortionShader()));
